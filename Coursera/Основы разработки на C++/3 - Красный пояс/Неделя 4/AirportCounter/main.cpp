...
AirportCounter<TAirport>::AirportCounter(TIterator begin, TIterator end)
{
  ...
  {
    m_items[static_cast<std::size_t>(*iter)].second++;
  }
}

template <typename TAirport>
std::size_t AirportCounter<TAirport>::Get(TAirport airport) const
{
  return m_items[static_cast<std::size_t>(airport)].second;
}

template <typename TAirport>
void AirportCounter<TAirport>::Insert(TAirport airport)
{
  m_items[static_cast<std::size_t>(airport)].second++;
}
...
{
  m_items[static_cast<std::size_t>(airport)].second = 0;
}

template <typename TAirport>
const typename AirportCounter<TAirport>::Items& AirportCounter<TAirport>::GetItems() const
{
  return m_items;
}

template <typename TAirport>
void AirportCounter<TAirport>::ClearItems()
{
  for (std::size_t i = 0; i < m_items.size(); ++i)
  {
    m_items[i].first = static_cast<TAirport>(i);
    m_items[i].second = 0;
  }
}
