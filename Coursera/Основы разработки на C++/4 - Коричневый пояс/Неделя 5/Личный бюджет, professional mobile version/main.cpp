...
MoneyState operator+ (const MoneyState& lhs, const MoneyState& rhs) {
  MoneyState result;
  result.earned = lhs.earned + rhs.earned;
  result.spent = lhs.spent + rhs.spent;
  return result;
}

struct BulkMoneyAdder {
  MoneyState delta = { 0.0, 0.0 };
};

//constexpr uint8_t TAX_PERCENTAGE = 13;

struct BulkTaxApplier {
  BulkTaxApplier() : factor(1.0)
  {}
  
  BulkTaxApplier(uint8_t tax_percantage) : factor(1.0 - tax_percantage / 100.0)
  {}
...
public:
  ...
  void CombineWith(const BulkLinearUpdater& other) {
    ...
    add_.delta.spent = add_.delta.spent + other.add_.delta.spent;
  }

  MoneyState Collapse(MoneyState origin, IndexSegment segment) const {
    MoneyState result;
    result.earned = origin.earned * tax_.factor + add_.delta.earned * segment.length();
    result.spent = origin.spent + add_.delta.spent * segment.length();
    return result;
  }
...
struct ComputeIncomeRequest : ReadRequest<double> {
  ...
  double Process(const BudgetManager& manager) const override {
    MoneyState result = manager.ComputeSum(MakeDateSegment(date_from, date_to));
...
struct SpendRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyAdder{MoneyState{0.0, daily_cost}});
  }
