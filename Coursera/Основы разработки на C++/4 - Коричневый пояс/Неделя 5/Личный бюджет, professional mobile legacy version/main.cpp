...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor, tree_values_spent, tree_spent;

void Init() {
  tree_values.fill(0);
  tree_add.fill(0);
  tree_factor.fill(1);
  tree_values_spent.fill(0);
  tree_spent.fill(0);
}
...
void Push(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      ...
      tree_values_spent[w] += tree_spent[v] * (r - l) / 2;
    }
  }
  tree_factor[v] = 1;
  tree_add[v] = 0;
...
/*void PushSpent(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      tree_spent[w] += tree_spent[v];
      tree_values_spent[w] += tree_spent[v] * (r - l) / 2;
...
double ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  ...
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    return tree_values[v] - tree_values_spent[v];
  }
...
void Spent(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    tree_spent[v] += value;
    tree_values_spent[v] += value * (r - l);
    return;
  }
  Spent(v * 2, l, (l + r) / 2, ql, qr, value);
  Spent(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
  tree_values_spent[v] =
    (v * 2 < VERTEX_COUNT ? tree_values_spent[v * 2] : 0)
    + (v * 2 + 1 < VERTEX_COUNT ? tree_values_spent[v * 2 + 1] : 0);
}
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    if (query_type == "ComputeIncome") {
      ...
      double value;
      cin >> value;
      Spent(1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from));
    }
  }
