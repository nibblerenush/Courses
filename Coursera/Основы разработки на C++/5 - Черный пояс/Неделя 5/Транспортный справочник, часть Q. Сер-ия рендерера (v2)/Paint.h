...
class Paint {
public:
  Paint(const RenderSettings& render_settings, const StopsStorage& stop_storage, const std::map<std::string_view, const Route&>& buses);
  void Render(std::ostream& out);
  void RenderRoute(
    std::ostream& out,
    const std::vector<BusElement>& bus_elems,
    const std::vector<WaitElement>& wait_elems);
...
private:
  ...
  const std::map<std::string_view, const Route&>& m_buses;
  ...
  const std::unordered_map<
        std::string,
        std::function<void(Svg::Document&, const std::vector<BusElement>&, const std::vector<WaitElement>&)>
  ...
  // Help info
  std::unordered_map<std::string_view, bool> m_reference_stops;
