...
void Paint::CreateLonLatIdx() {
  for (const auto& [_, value] : m_stop_storage.GetStops()) {
    m_lon_idx.emplace(value.geo_coords.longitude, 0);
    m_lat_idx.emplace(value.geo_coords.latitude, 0);
  }

  std::size_t idx = 0;
  std::for_each(m_lon_idx.begin(), m_lon_idx.end(), [&idx](auto& item) {
    item.second = idx++;
  });

  idx = 0;
  std::for_each(m_lat_idx.begin(), m_lat_idx.end(), [&idx](auto& item) {
    item.second = idx++;
  });
}
...
void Paint::CalcXyStep() {
  if (m_lon_idx.size() > 1) {
    m_x_step = (m_render_settings.width - 2 * m_render_settings.padding) / (m_lon_idx.size() - 1);
  }

  if (m_lat_idx.size() > 1) {
    m_y_step = (m_render_settings.height - 2 * m_render_settings.padding) / (m_lat_idx.size() - 1);
  }
}

double Paint::GetX(double longitude) const {
  return m_lon_idx.at(longitude) * m_x_step + m_render_settings.padding;
}

double Paint::GetY(double latitude) const {
  return m_render_settings.height - m_render_settings.padding - m_lat_idx.at(latitude) * m_y_step;
}
