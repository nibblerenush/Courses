...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count) : m_mutex_vec(bucket_count), m_unordered_map_vec(bucket_count)
  {}

  WriteAccess operator[](const K& key)
  {
    size_t vec_index = GetVecIndex(key);
    return WriteAccess{ lock_guard(m_mutex_vec[vec_index]), m_unordered_map_vec[vec_index][key] };
  }

  ReadAccess At(const K& key) const
  {
    size_t vec_index = GetVecIndex(key);
    return ReadAccess{ lock_guard(m_mutex_vec[vec_index]), m_unordered_map_vec[vec_index].at(key) };
  }
  ...
  bool Has(const K& key) const
  {
    size_t vec_index = GetVecIndex(key);
    {
      lock_guard guard(m_mutex_vec[vec_index]);
      return m_unordered_map_vec[vec_index].count(key);
    }
  }
  ...
  {
    MapType result;
    for (size_t i = 0; i < m_unordered_map_vec.size(); ++i)
    {
      lock_guard<mutex> guard(m_mutex_vec[i]);
      result.insert(m_unordered_map_vec[i].begin(), m_unordered_map_vec[i].end());
    }
    return result;
  }

private:
  size_t GetVecIndex(const K& key) const
  {
    return hasher(key) % m_unordered_map_vec.size();
  }

private:
  mutable vector<mutex> m_mutex_vec;
  vector<MapType> m_unordered_map_vec;
  Hash hasher;
};
