...
private:
  ...
  struct Proxy
  {
    ...
    void operator= (const T& new_coeff)
    {
      // Not new alloc
      if (cur_index < coeffs.size())
      {
        coeffs[cur_index] = new_coeff;
        if (cur_index == coeffs.size() - 1 && new_coeff == 0)
        {
          auto iter = find_if(coeffs.rbegin(), coeffs.rend(), [](const T& coeff) { return coeff != 0; });
          if (iter != coeffs.rend())
          {
            coeffs.resize(coeffs.size() - distance(coeffs.rbegin(), iter));
          }
          else
          {
            coeffs.resize(1);
    ...
    operator T() const
    {
      ...
      return coeffs[cur_index];
    }
  };

  Proxy proxy_{ coeffs_ };
