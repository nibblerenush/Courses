...
namespace Svg {
  ...
  public:
    ...
    const std::variant<std::string, Rgb, Rgba>& Internal() const;
  private:
    ColorType m_type = ColorType::STR;
  ...
  public:
    Rectangle& SetLeftCorner(Point left_corner);
  ...
  private:
    std::vector<std::variant<Circle, Polyline, Text, Rectangle>> m_objects;
  };
}
