...
public:
  void ReadRoutingSettings(const Json::Node& root_node) {
    ...
    m_routing_settings.bus_wait_time = static_cast<int>(json_routing_settings.AsMap().at("bus_wait_time").AsNumber());
    m_routing_settings.bus_velocity = json_routing_settings.AsMap().at("bus_velocity").AsNumber() * 1000.0 / 60.0;
  ...
  void Serialize() {
    ...
    for (const auto& [_, route] : m_buses) {
      ...
      bus_response.set_bus_name(route.GetName());
      ...
      for (const auto& stop : route.GetStops()) {
        *bus_response.add_stops() = stop.stop_name;
      }
      
      if (route.GetType() == RouteType::CIRCLE) {
        bus_response.set_is_roundtrip(true);
      }
      else {
        bus_response.set_is_roundtrip(false);
      }
    ...
    for (const auto& stop : m_stops_storage.GetStops()) {
      ...
      for (const auto& bus_name : stop.second.bus_names) {
        *stop_response.add_buses() = bus_name;
      }

      // extra
      Serialize::GeoCoords proto_geo_coords;
      proto_geo_coords.set_latitude(stop.second.geo_coords.latitude);
      proto_geo_coords.set_longitude(stop.second.geo_coords.longitude);
      *stop_response.mutable_geo_coords() = move(proto_geo_coords);
      
      *transport_catalog.add_stop_responses() = move(stop_response);
    }
    
    // stop_numbers
    std::unordered_map<string_view, int> stop_number;
    for (const auto& [stop_name, stop_info] : m_stops_storage.GetStops()) {
      Serialize::StopNumber proto_stop_number;
      proto_stop_number.set_stop_number(stop_info.number);
      proto_stop_number.set_stop_name(stop_name);
      stop_number[stop_name] = stop_info.number;
      *transport_catalog.add_stop_numbers() = move(proto_stop_number);
    }
    ...
    for (const auto& from_stop : m_stops_storage.GetStops()) {
      for (const auto& to_stop : m_stops_storage.GetStops()) {
        ...
        if (result.has_value()) {
          ...
          route_response.set_from_stop_number(from_stop.second.number);
          route_response.set_to_stop_number(to_stop.second.number);
          ...
          for (size_t i = 0; i < result.value().edge_count; ++i) {
            ...
            if (edge.weight.wait_time != 0) {
              Serialize::RouteElement wait_element;
              wait_element.set_type(Serialize::RouteElement_RouteElementType_WAIT_ELEMENT);
              wait_element.set_stop_number(stop_number[string{edge.weight.from_stop_name}]);
              *route_response.add_route_elements() = move(wait_element);
            }
            ...
            bus_element.set_bus(string{edge.weight.bus_name});
            bus_element.set_span_count(edge.weight.count);
            bus_element.set_from_idx(edge.weight.from_idx);
            bus_element.set_to_idx(edge.weight.to_idx);
    ...
    if (transport_catalog.ParseFromIstream(&input_file)) {
      ...
      for (const auto& proto_stop_response : transport_catalog.stop_responses()) {
        ...
        GeoCoords stop_geo_coords = {
          proto_stop_response.geo_coords().latitude(),
          proto_stop_response.geo_coords().longitude()
        };
        m_stops_storage.Add(proto_stop_response.stop_name(), stop_geo_coords);
      }
      ...
      for (const auto& proto_bus_response : transport_catalog.bus_responses()) {
        ...
        const std::string& bus_name = proto_bus_response.bus_name();
        m_routes.push_back(Route(bus_name));
        Route& route = m_routes.back();

        for (const auto& stop_name : proto_bus_response.stops()) {
          route.AddStop(stop_name, m_stops_storage);
          m_stops_storage.AddBus(stop_name, route.GetName());
        }

        if (proto_bus_response.is_roundtrip()) {
          route.CommonSetType(RouteType::CIRCLE);
        }
        else {
          route.CommonSetType(RouteType::LINEAR);
        }
      ...
      // number_stop
      unordered_map<int, string> number_stop;
      for (const auto& proto_stop_number : transport_catalog.stop_numbers()) {
        number_stop[proto_stop_number.stop_number()] = proto_stop_number.stop_name();
      }
      ...
      for (const auto& proto_route_response : transport_catalog.route_responses()) {
        const string& from = number_stop[proto_route_response.from_stop_number()];
        const string& to = number_stop[proto_route_response.to_stop_number()];

        for (const auto& proto_route_element : proto_route_response.route_elements()) {
          switch (proto_route_element.type()) {
          case Serialize::RouteElement_RouteElementType_WAIT_ELEMENT:
            m_route_responses[{from, to}].push_back(WaitElementSer{number_stop[proto_route_element.stop_number()]});
            break;
          case Serialize::RouteElement_RouteElementType_BUS_ELEMENT:
            m_route_responses[{from, to}].push_back(BusElementSer{
              proto_route_element.time(),
              proto_route_element.bus(),
              proto_route_element.span_count(),
              static_cast<size_t>(proto_route_element.from_idx()),
              static_cast<size_t>(proto_route_element.to_idx())});
            break;
          }
        }
      ...
      m_render_settings.underlayer_width = proto_render_settings.underlayer_width();
    ...
    if (m_route_responses.count({from, to})) {
      if (!m_route_responses.at({from, to}).empty()) {
        ...
        for (const auto& route_element : m_route_responses.at({from, to})) {
          try {
            const WaitElementSer& time_element = get<WaitElementSer>(route_element);
            output_stream
              << Space(6) << "{\n"
              << Space(8) << "\"time\": " << m_routing_settings.bus_wait_time << ",\n"
              << Space(8) << "\"stop_name\": " << '\"' << time_element.stop_name << "\",\n"
              << Space(8) << "\"type\": " << "\"Wait\"\n";
            wait_elems.push_back(WaitElement{time_element.stop_name});
          }
            ...
            output_stream
              ...
              << Space(8) << "\"type\": " << "\"Bus\"\n";
            bus_elems.push_back(BusElement{bus_element.bus, bus_element.from_idx, bus_element.to_idx});
          }
...
private:
  ...
  unordered_map<FromTo, vector<variant<WaitElementSer, BusElementSer>>, Hasher> m_route_responses;
