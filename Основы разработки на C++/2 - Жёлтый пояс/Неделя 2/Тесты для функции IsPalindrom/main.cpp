#include <sstream>
#include <exception>
#include <iostream>
#include <string>
#include <map>
#include <set>

using namespace std;

template <class T>
ostream& operator << (ostream& os, const set<T>& s) {
  os << "{";
  bool first = true;
  for (const auto& x : s) {
    if (!first) {
      os << ", ";
    }
    first = false;
    os << x;
  }
  return os << "}";
}

template <class K, class V>
ostream& operator << (ostream& os, const map<K, V>& m) {
  os << "{";
  bool first = true;
  for (const auto& kv : m) {
    if (!first) {
      os << ", ";
    }
    first = false;
    os << kv.first << ": " << kv.second;
  }
  return os << "}";
}

template<class T, class U>
void AssertEqual(const T& t, const U& u,
  const string& hint)
{
  if (t != u) {
    ostringstream os;
    os << "Assertion failed: " << t << " != " << u
      << " hint: " << hint;
    throw runtime_error(os.str());
  }
}

inline void Assert(bool b, const string& hint) {
  AssertEqual(b, true, hint);
}

class TestRunner {
public:
  template <class TestFunc>
  void RunTest(TestFunc func, const string& test_name) {
    try {
      func();
      cerr << test_name << " OK" << endl;
    }
    catch (runtime_error& e) {
      ++fail_count;
      cerr << test_name << " fail: " << e.what() << endl;
    }
  }

  ~TestRunner() {
    if (fail_count > 0) {
      cerr << fail_count << " unit tests failed. Terminate" << endl;
      exit(1);
    }
  }

private:
  int fail_count = 0;
};

bool IsPalindrom(std::string str)
{
  if (str.length() == 0 || str.length() == 1)
  {
    return true;
  }

  int head = 0;
  int tail = str.length() - 1;

  while (head < tail)
  {
    if (str[head++] != str[tail--])
    {
      return false;
    }
  }

  return true;
}

void TestIsPalindrom()
{
  Assert(IsPalindrom(std::string()), "");
  Assert(IsPalindrom("abQba"), "");
  Assert(IsPalindrom("abba"), "");
  Assert(IsPalindrom("aba"), "");
  Assert(IsPalindrom("a"), "");
  Assert(IsPalindrom(""), "");

  Assert(!IsPalindrom("ab"), "");
  Assert(!IsPalindrom("abc"), "");
  Assert(!IsPalindrom("QabbaL"), "");
  Assert(!IsPalindrom("Qabba"), "");
  Assert(!IsPalindrom("abbaL"), "");
  Assert(!IsPalindrom("abQLba"), "");
  
  Assert(!IsPalindrom("a a "), "");
  Assert(IsPalindrom(" a a "), "");
  Assert(IsPalindrom(" "), "");
  Assert(IsPalindrom("  "), "");
  Assert(IsPalindrom("   "), "");
}

void TestAll()
{
  TestRunner tr;
  tr.RunTest(TestIsPalindrom, "TestIsPalindrom");
}

int main()
{
  TestAll();
  return 0;
}
