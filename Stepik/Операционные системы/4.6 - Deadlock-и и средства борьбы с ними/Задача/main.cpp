...
unsigned long long atomic_fetch_add(atomic_ullong*, int) {
  return 0;
}
...
void wait(struct condition*, struct lock*) {};
void notify_one(struct condition*) {};
void notify_all(struct condition*) {};
...
void ChangeOwner(struct wdlock* l, struct wdlock_ctx* ctx) {
  l->owner = ctx;
  
  struct wdlock* old_head = ctx->locks;
  l->next = old_head;
  ctx->locks = l;
}
...
void wdlock_unlock(struct wdlock_ctx* ctx) {
  struct wdlock* cur_lock = ctx->locks;
  
  while (cur_lock) {
    lock(&cur_lock->lock);
    
    cur_lock->owner = NULL;
    notify_one(&cur_lock->cv);
    
    unlock(&cur_lock->lock);
