...
bool CheckBinaryTree(Node* root, std::int64_t min, std::int64_t max)
{
  if (root == nullptr) {
    return true;
  }
...
int main()
{
  ...
  if (CheckBinaryTree(root, std::numeric_limits<std::int64_t>::min(), std::numeric_limits<std::int64_t>::max())) {
    std::cout << "CORRECT";
  }
  else {
    std::cout << "INCORRECT";
